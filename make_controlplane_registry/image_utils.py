...
def get_image(name: str, repository: str = None):
    ...
    if name.startswith("quay.io/jetstack"):
        new_image_name = name.replace("quay.io/jetstack", repository)
    if name.startswith("registry.k8s.io/cluster-api"):
        new_image_name = name.replace("registry.k8s.io/cluster-api", repository)
    if name.startswith("registry.k8s.io/capi-openstack"):
        new_image_name = name.replace("registry.k8s.io/capi-openstack", repository)
